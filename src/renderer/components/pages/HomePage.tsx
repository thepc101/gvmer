...
export function HomePage({ onSelectGame }: HomePageProps) {
  ...
  useEffect(() => {
    window.gvmer?.getGames().then((allGames) => {
      setGames(allGames);
      setLoaded(true);
    });
  }, []);
          ...
          {continueGame ? (
            ...
            <div className="w-full aspect-[16/9] bg-border rounded-2xl flex items-center justify-center">
              <div className="text-center">
                <p className="text-sm text-secondary">No games detected yet</p>
                <button
                  onClick={async () => {
                    await window.gvmer?.scanGames();
                    const g = await window.gvmer?.getGames();
                    setGames(g);
                  }}
      ...
      >
        ...
        <p className="text-xs text-secondary mt-2">
          {games.length > 0
            ...
            : loaded ? "No games found. Click Scan to discover your library." : "Loading..."}
        </p>
      </motion.div>
    </div>
  );
